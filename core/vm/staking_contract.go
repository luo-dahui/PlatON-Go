...
func (stkc *StakingContract) createStaking(typ uint16, benefitAddress common.Address, nodeId discover.NodeID,
	...
	*/
	...
	canMutable := &staking.CandidateMutable{
		...
		WithdrewDelegateAmount: new(big.Int).SetInt64(0),
	}
...
func (stkc *StakingContract) editCandidate(benefitAddress *common.Address, nodeId discover.NodeID, rewardPer *uint16,
	...
	err = stkc.Plugin.EditCandidate(blockHash, blockNumber, canAddr, canOld, state)
	if nil != err {
		if bizErr, ok := err.(*common.BizError); ok {
			return txResultHandler(vm.StakingContractAddr, stkc.Evm, "editCandidate",
...
func (stkc *StakingContract) withdrewDelegation(stakingBlockNum uint64, nodeId discover.NodeID, amount *big.Int) ([]byte, error) {
	...
	if gov.Gte0160VersionState(state) {
		err = stkc.Plugin.WithdrewDelegation(state, blockHash, blockNumber, amount, from, nodeId, stakingBlockNum, del, delegateRewardPerList)
		if nil != err {
			if bizErr, ok := err.(*common.BizError); ok {
				return txResultHandler(vm.StakingContractAddr, stkc.Evm, "withdrewDelegation",
...
func (stkc *StakingContract) redeemDelegation(stakingBlockNum uint64, nodeId discover.NodeID) ([]byte, error) {
	...
	if !gov.Gte0160VersionState(state) {
		log.Warn("The current version does not support this interface", "txHash", txHash.Hex(), "blockNumber", blockNumber.Uint64(),
			"delAddr", from, "nodeId", nodeId.String(), "stakingNum", stakingBlockNum, "curVersion", params.FormatVersion(gov.GetCurrentActiveVersion(state)))
		return nil, nil
	}
	...
	issueIncome, err := stkc.Plugin.RedeemDelegation(state, blockHash, blockNumber, txHash, from, nodeId, stakingBlockNum, del, delegateRewardPerList)
	if nil != err {
		if bizErr, ok := err.(*common.BizError); ok {
			return txResultHandler(vm.StakingContractAddr, stkc.Evm, "redeemDelegation",
